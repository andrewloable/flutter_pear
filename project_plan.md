...
**iOS shipped in v0.2, SIMULATOR-VALIDATED** — SPM-first (`Package.swift`, generated by `:pack` from the `barekit-pin.json` pin manifest, with a `url:` `binaryTarget` for BareKit) plus a compat CocoaPods podspec, superseding the earlier "CocoaPods podspec" plan (CEO-level supersession decision — SPM is Flutter's own plugin-resolution default as of 3.44+, and the compat podspec exists only for consumers who haven't migrated). BareKit's own binaries are fetched at consumer build time from the committed pin manifest — a two-link checksum chain: upstream `holepunchto/bare-kit` v2.3.0 ships one multi-platform `prebuilds.zip`, which this repo's maintainer re-packs into a single-xcframework asset and republishes (SwiftPM's `binaryTarget` needs one ready-made zip, not a build-time extraction step). The compat podspec's own BareKit fetch runs in a `script_phase`, never `prepare_command` — CocoaPods issue 2187: `prepare_command` never runs for `:path`-installed pods, which is how Flutter installs every plugin pod. Addon xcframeworks (the same Hyper\* native addon set Android's `jniLibs` carries) are committed under `packages/flutter_pear_bare/ios/addons/`, versioned like source — a fresh checkout never needs `:pack` to build. iOS is not a gate on the v0.1 Android release; it shipped as its own v0.2 milestone. iOS aggressively suspends background sockets regardless of anything this plugin does — see `packages/flutter_pear/doc/ios.md` for the honest background-execution story, including the native suspend fix that transitions cleanly without extending background connectivity.
...
**Install must be one line.** `flutter pub add flutter_pear` and it works. Native Bare Kit binaries and the pear-end bundle resolve via a Gradle task (Android) + SwiftPM with a `script_phase`-based CocoaPods compat path (iOS, shipped in v0.2). *Zero* manual NDK/ABI/podfile edits on either platform. Verify by hand, before release (this project has no CI): create a new Flutter app, add the package, and build it on both platforms.
...
**Time-to-first-success: champion tier, under 5 minutes.** Committed target (not the earlier draft's 15-minute goal): clone → `melos bootstrap` → `flutter run` × 2 → QR scan → message on both screens, under 5 minutes. Enforced by a manual, by-hand TTHW budget check before each release (`packages/flutter_pear/tool/fresh_machine_check.sh`, E9.3 — this project has no CI) plus a quarterly `/devex-review` boomerang on the live package (a human run can't be automated away — device auth prompts, cold caches). One-phone devs reach the same moment via the desktop CLI peer (E7/X7).
...
**Tooling & governance.** melos monorepo; CHANGELOG; issue templates ("include `flutter doctor` + worklet log"); pin the pear-end module versions and publish a compatibility table (plugin ↔ Bare Kit ↔ Hyper* versions) checked by hand before release (`check_compatibility.dart`, E9.5 — this project has no CI); "unofficial, not affiliated with Holepunch" in README; MIT/Apache-2.0 license to match ecosystem. Publishing itself is a manual, by-hand process — dry-run publish all three packages in order, then real publish (E9.7) — see §9 for why this replaced the earlier semantic-release/CI-workflow idea.
...
- **Semantic-release — DROPPED.** Superseded by E9.7's tag-triggered publish workflow (tag → dry-run all three packages in dependency order → manual approval gate → real publish). That mechanism already gives ordered, reviewable releases without an additional commit-message-driven versioning tool; adding one on top would be redundant process for a small monorepo. **SUPERSEDED AGAIN (2026-07-06):** GitHub Actions was removed from this repo entirely (developer decision, after real CI infra problems) — publishing is now a fully manual, by-hand `dart pub publish` sequence with no workflow/approval-gate automation at all; the ordering/dry-run discipline itself is unchanged, just no longer machine-enforced.
- **"Track upstream releases in CI" — DROPPED as a dedicated automated job.** The compatibility/toolchain table (E9.5) already gives a CI-enforced, manually-bumped contract between the plugin, Bare Kit, and the Hyper* family; the bump procedure it documents is the intended way version drift gets noticed and handled. A separate renovate/dependabot-style bot for a two-surface pin (Bare Kit + pear-end JS deps) isn't worth the maintenance overhead right now — revisit only if the manual bump procedure demonstrably misses a break. **SUPERSEDED (2026-07-06):** "CI-enforced" above is now "checked by hand before every push" — see the semantic-release entry's own superseding note; `check_compatibility.dart` itself is unchanged, only its automation is gone.
- **M0 "post intent on `holepunchto/bare-kit` discussions" — DROPPED.** This was meant to surface blockers *before* investing in the risky M1 spike. That spike (E1) is done and passed (worklet boots, joins, relays on Android, real-emulator-verified); the moment this action was meant to serve has passed. Nothing prevents posting about the project once it's further along, but it's no longer a plan item with its own acceptance criteria.
