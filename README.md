...
3. **The macOS Deployment Target is 12.0+** in `macos/Runner.xcodeproj` (or in Xcode: Runner target → General → Minimum Deployments). Current Flutter templates already default to 12.0, so a freshly created app needs nothing here; an older project carried forward may still sit at 10.15 and will fail immediately with a raw SwiftPM error (`requires minimum platform version 12.0`) that names no flutter_pear file or fix at all. **Raised from 10.15.4 in 0.4.2** — Xcode 27 refuses to target macOS below 12.0 at all, so the old floor was unbuildable, not merely untested.
