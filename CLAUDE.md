...
E9 (CI/CD + publishing) is done: `flutter_pear`, `flutter_pear_bare`, and `flutter_pear_test` are all published on pub.dev, currently at **v0.4.1**, released through `publish.yml` rather than by hand. **This repo runs no push/PR CI** (deliberate decision, developer instruction — removed after real infra problems; see `COMPATIBILITY.md`'s header), so every quality gate (`melos run analyze`/`test`, `check_compatibility.dart`, pana, license enforcement) is run by hand during development. The single workflow that does exist, `.github/workflows/publish.yml`, triggers on a `v*.*.*` tag push and on **nothing else** — no branch trigger, no `pull_request`, no `workflow_dispatch` — and runs analyze/test/`check_compatibility`/`check_pins` plus a dry-run before the three real pub.dev publishes (OIDC trusted publishing, gated behind a required-reviewer `pub-dev-release` environment). A commit to main never starts a build. Run `bd list --status=open` / `bd ready` for exact live status — this file is a snapshot, bd is the source of truth.
...
**0.4.0 raised Android's floor to `minSdk` 29 (Android 10) — consumer-breaking.** Bare Kit moved 2.3.0 → 2.5.5. The old `minSdk = 24` was *false*: Bare Kit 2.3.0 linked `libnativehelper.so` and imported `JNI_GetCreatedJavaVMs@LIBNATIVEHELPER_S`, a non-weak symbol whose version tag doesn't exist before Android 12, so apps on 24–30 built and installed fine and then threw `UnsatisfiedLinkError` on the first `Pear.start()`. Upstream removed that link in 2.5.0 ([bare-kit#115](https://github.com/holepunchto/bare-kit/pull/115)), which is what makes API 29 reachable at all. Validated at runtime on an API 29 arm64 emulator (live `bare-worklet` thread, real pairing invite). **Every consuming app must set `minSdk = 29` itself** — the plugin cannot do it for them, and below 29 the Gradle manifest merger fails the build. When bumping Bare Kit again, re-read the ELF API note (`.note.android.ident`) rather than assuming the floor still holds; see the comment on `minSdk` in `flutter_pear_bare/android/build.gradle`.

0.4.1 is a docs/housekeeping patch: both READMEs still advertised v0.3.1 and promised "zero build-wiring steps", which the `minSdk` requirement had made false.
...
## Writing bd task descriptions for low-context implementers

A bd issue may get picked up by an agent with none of the conversation context that produced
it — no design-discussion memory, no chance to ask a quick clarifying question mid-thought.
Write every `--description`/`--acceptance` as if handing it to that agent cold. Concretely:

- **Acceptance criteria must be objective and testable**, not a restated goal. "Harness exits
  non-zero the instant any non-loopback socket is opened, or if B's bytes != A's" is
  implementable without guessing; "make sure it's secure" is not.
- **Name exact file paths** for what gets created or changed, not just a feature name.
- **Reference the source design/decision doc by path** (e.g. a `~/.gstack/projects/.../*.md`
  design doc, or a section of `project_plan.md`) if one exists — link the *why*, don't
  re-explain it inline every time.
- **State environmental/hardware/credential prerequisites explicitly, and what to do if one is
  missing at implementation time** — don't leave it implicit. If a task needs a physical
  device, a paid API key, or a running external service that might not be available, say so
  and state the fallback (downgrade the acceptance bar and document it as a developer
  decision, defer the issue, or flag for a human — see `flutter_pear-doi`'s emulator-downgrade
  precedent). Without this, a hardware gap becomes a mid-task blocker instead of a known,
  pre-decided path.
- **State what happens to throwaway/intermediate artifacts on completion** — e.g. "this spike
  script gets deleted once its numbers are written up" vs. "this becomes a permanent file."
  Leaving this unstated forces a guess about whether to delete real, working code.
- **Spell out non-obvious constraints the implementer must not violate** (an architecture
  invariant, a licensing rule, an ordering requirement) rather than assuming they're inferred
  from context — e.g. "never construct HyperDHT/`new Hyperswarm()`" mattered enough to the
  correctness of one spike that it was worth stating up front, not discovered by trial and
  error.
- **Make dependencies explicit with `bd dep add`** so ordering isn't left to inference.
- Avoid vague verbs ("improve," "handle," "support") with no concrete definition of "done."
