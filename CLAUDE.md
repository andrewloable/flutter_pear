...
E9 (CI/CD + publishing) is open but nearly done: `flutter_pear`, `flutter_pear_bare`, and `flutter_pear_test` are all published on pub.dev at v0.0.1. **This repo runs no push/PR CI** (deliberate decision, developer instruction — removed after real infra problems; see `COMPATIBILITY.md`'s header), so every quality gate (`melos run analyze`/`test`, `check_compatibility.dart`, pana, license enforcement) is run by hand during development. The single workflow that does exist, `.github/workflows/publish.yml`, triggers on a `v*.*.*` tag push and on **nothing else** — no branch trigger, no `pull_request`, no `workflow_dispatch` — and runs analyze/test/`check_compatibility`/`check_pins` plus a dry-run before the three real pub.dev publishes (OIDC trusted publishing, gated behind a required-reviewer `pub-dev-release` environment). A commit to main never starts a build. What's left on E9: the publish-day checklist's remaining steps (GitHub Discussions/issue templates, final post-publish rendering/smoke confirmation). Run `bd list --status=open` / `bd ready` for exact live status — this file is a snapshot, bd is the source of truth.
